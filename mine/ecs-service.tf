...
data "aws_secretsmanager_secret" "database_uri" {
  name = "platform-${var.env}-database-uri"
}
...
module "ecs-task" {
  ...
  name   = "platform-${var.env}-${var.container_name}"
  image  = var.image
  ...
  environment = [
    ...
    },
    {
      ...
      value = var.elasticache_address == null ? module.redis[0].endpoint : var.elasticache_address
    },
    {
      name  = "COGNITO_DOMAIN",
    ...
    },
 {
  ...
  #  {
  #    name = "DATABASE_URI"
  #    value = data.aws_secretsmanager_secret_version.database_uri.secret_string
  #  },
    {
...
module "ecs-service" {
  source                         = "terraform-aws-modules/ecs/aws/service"
  name                           = "platform-${var.env}-${var.service_name}"
  container_definition_json      = module.ecs-task.json_map_encoded_list
  ...
  ecs_load_balancers = [
    {
      ...
      elb_name         = "platform-${var.env}-${var.elb_name}"
